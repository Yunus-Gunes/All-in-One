...
const wrongLetters = [];
let selectedWord = getRandWord();

// Rastgele kelime seçmek
function getRandWord() {
    const words = ['python', 'javascript', 'css', 'html'];
    return words[Math.floor(Math.random() * words.length)];
}

// Kelimeyi ekrana yazdırmak
...
function updateWrongLetters() {
    wrongLetters_el.innerHTML = `
        ${wrongLetters.length > 0 ? '<p>Yanlış Harfler</p>' : ''}
